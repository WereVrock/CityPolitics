 ...
 * status, and the Bank's secondary mercenary manpower pool.
 */
...
private void defaultLoan(NobleHouse house, BankLoan loan, List<String> log) {
     ...
     * to fold combat power into the battle) or abandon the Bank. The
     * aggressor is permanently branded a Bank-Robber.
     */
...
private boolean decideStakeholderDefense(NobleHouse stakeholder, NobleHouse bank, int aggressorPower) {
    ...
    public void growMercenaryManpower(List<NobleHouse> allHouses, int playerManpower) {
        ...
        int cap = (int) (totalManpower * BankParams.BANK_MERC_MANPOWER_GROWTH_FRACTION_OF_TOTAL);
        if (mercenaryManpower < cap) mercenaryManpower++;
    }

package City.main.parameters;

public final class CommanderParams {

    public static final int COMMANDER_RECRUIT_OPINION_GAIN = City.main.parameters.levels.OpinionLevels.MEDIUM;
    public static final int COMMANDER_POOL_REFRESH_SIZE = 3;
    public static final double[] COMMANDER_UPKEEP_BY_SKILL = {0.8, 1.2, 1.5, 2.0};
    public static final int COMMANDER_DISMISS_COST = City.main.parameters.levels.InfluenceLevels.LOW;
    // =========================================================
    // ── COMMANDERS (CommanderParams) ────────────────────────
    // =========================================================
    public static final int COMMANDER_POOL_BASE_SIZE = 3;
    public static final double COMMANDER_OVERCAP_INFLUENCE_COST = 0.5;
    public static final int COMMANDER_SKILL_WEIGHT_2 = 95;
    public static final int COMMANDER_RECRUIT_BASE_COST = City.main.parameters.levels.InfluenceLevels.LOW;
    public static final int COMMANDER_POOL_REFRESH_COST = City.main.parameters.levels.InfluenceLevels.LOW;
    public static final int COMMANDER_SKILL_WEIGHT_1 = 80;
    public static final int COMMANDER_DISMISS_OPINION_LOSS = City.main.parameters.levels.OpinionLevels.MEDIUM;
    public static final int[] COMMANDER_XP_THRESHOLDS = {100, 250, 500};
    public static final int COMMANDER_SKILL_WEIGHT_0 = 25;
    public static final int COMMANDER_PARTY_POWER_PER_ALIVE = 10;
    public static final int COMMANDER_FREE_CAP = 3;
    // all constants will be moved here
}
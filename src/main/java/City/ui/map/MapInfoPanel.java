...
public class MapInfoPanel extends JPanel {
    ...
    private java.util.Set<String>          unlawfulPickerValidIds  = null;
    private java.util.function.Consumer<String> unlawfulPickerCallback = null;

    // ── Army card components ──
    ...
    public void showZone(Zone zone) {
        ...
        // Unlawful picker mode: intercept zone display
        if (unlawfulPickerValidIds != null) {
            showZoneInPickerMode(zone);
            return;
        }
        ...
        City.main.nobles.NobleHouse zoneOwner = nobleHouseManager.getOwnerOfZone(zone.getId());
        if (zoneOwner != null) {
        ...
        if (ravagedZoneManager != null) {
            City.main.barbarians.RavagedZoneManager.RavagedLevel lvl =
                ravagedZoneManager.getLevel(zone.getId());
            if (lvl == City.main.barbarians.RavagedZoneManager.RavagedLevel.HEAVILY_RAVAGED) {
                damageLabel.setText(damageLabel.getText() + "   ☠ HEAVILY RAVAGED");
                damageLabel.setForeground(new java.awt.Color(220, 40, 40));
            } else if (lvl == City.main.barbarians.RavagedZoneManager.RavagedLevel.RAVAGED) {
    ...
    public void showArmy(City.main.army.Army army, ZoneManager zm) {
    ...
    public void showNobleArmy(City.main.nobles.NobleArmy army, ZoneManager zm,
                               City.main.nobles.NobleHouseManager houseManager) {
        if (army == null) { clearArmy(); return; }
        City.main.nobles.NobleHouse owner = houseManager.getHouseById(army.getHouseId());
    ...
    public void showBarbArmy(City.main.barbarians.BarbArmy army,
                              City.main.core.GameState gameState) {
        if (army == null) { clearArmy(); return; }
        ...
        City.main.map.Zone zone = gameState.getZoneManager().getZone(army.getZoneId());
        armyZoneLabel.setText("📍 " + (zone != null ? zone.getDisplayName() : army.getZoneId()));
        ...
        if (canPayOff) {
            ...
            barbPayOffBtn.addActionListener(e -> {
                ledger.applyOneTime(City.main.resources.ResourceType.GOLD, "barbarians", "Pay Off", -goldCost, res);
                ledger.applyOneTime(City.main.resources.ResourceType.FOOD, "barbarians", "Pay Off", -foodCost, res);
        ...
        if (canDismiss) {
            ...
            barbDismissBtn.addActionListener(e -> {
                ledger.applyOneTime(City.main.resources.ResourceType.GOLD, "barbarians", "Dismiss", -fullCost, res);
                army.dismiss();
                clearArmy();
            });
        }
    ...
    private void showZoneInPickerMode(Zone zone) {
        ...
        for (java.awt.event.ActionListener al : ownerButton.getActionListeners())
            ownerButton.removeActionListener(al);
        City.main.nobles.NobleHouse owner = nobleHouseManager.getOwnerOfZone(zone.getId());
        ownerButton.setText(owner != null ? "Owner: " + owner.getName() : "Unowned");
        ownerButton.setForeground(UITheme.TEXT_SECONDARY);
        ...
        if (valid) {
            for (City.main.nobles.NobleHouse h : nobleHouseManager.getHouses()) {
                if (nobleHouseManager.getClaimManager().hasClaim(h.getId(), zone.getId())) {
                    JLabel lbl = new JLabel("  ⚑ " + h.getName().replace("House ", ""));
                    lbl.setFont(UITheme.FONT_SMALL);
                    lbl.setForeground(new java.awt.Color(200, 180, 100));
                    claimsPanel.add(lbl);
                }
            }
        }
        ...
        // Select button
        adjacentArea.setText(valid ? "Click SELECT to choose this zone." : "This zone cannot be selected.");
        adjacentArea.setForeground(valid ? UITheme.TEXT_GOLD : UITheme.TEXT_SECONDARY);

        // Inject select button via barbPayOffBtn reuse
        barbPayOffBtn.setVisible(valid);
        barbDismissBtn.setVisible(false);
        if (valid) {
            for (java.awt.event.ActionListener al : barbPayOffBtn.getActionListeners())
                barbPayOffBtn.removeActionListener(al);
            barbPayOffBtn.setText("SELECT THIS ZONE");
            barbPayOffBtn.setForeground(UITheme.TEXT_GOLD);
            final String capturedId = zone.getId();
            barbPayOffBtn.addActionListener(e -> {
                if (unlawfulPickerCallback != null) unlawfulPickerCallback.accept(capturedId);
            });
        }
        ...
        commanderSkillLabel.setVisible(false);
        openMilitaryBtn.setVisible(false);
        cardLayout.show(cards, CARD_ARMY); // reuse army card layout for the buttons
    }

    private void showHouseDialog(City.main.nobles.NobleHouse house) {
    ...
    private void showCommanderDialog(City.main.army.commander.Commander cmd) {
        Window parent = SwingUtilities.getWindowAncestor(this);
        JDialog dialog = new JDialog(parent instanceof Frame ? (Frame) parent : null,
                cmd.getName(), true);
    ...
    private void updateClaimsPanel(String zoneId) {
        claimsPanel.removeAll();
        java.util.List<City.main.nobles.NobleHouse> claimants = new java.util.ArrayList<>();
        for (City.main.nobles.NobleHouse h : nobleHouseManager.getHouses()) {
        ...
        if (claimants.isEmpty()) {
            ...
            for (City.main.nobles.NobleHouse h : claimants) {
    ...
    /**
     * Activates unlawful-zone-picker mode.
     * When active, showZone() shows a "Select this zone" button for valid zones
     * and a greyed indicator for invalid ones.
     */
    public void setUnlawfulPickerMode(java.util.Set<String> validIds,
                                       java.util.function.Consumer<String> onPick) {
        this.unlawfulPickerValidIds  = validIds;
        this.unlawfulPickerCallback  = onPick;
    }

    public void clearUnlawfulPickerMode() {
        this.unlawfulPickerValidIds  = null;
        this.unlawfulPickerCallback  = null;
    }

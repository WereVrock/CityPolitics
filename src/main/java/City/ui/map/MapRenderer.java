...
import static City.main.map.Zone.SettlementType.CAPITAL;
import static City.main.map.Zone.SettlementType.DESOLATE;
import static City.main.map.Zone.SettlementType.TOWN;
import static City.main.map.Zone.SettlementType.VILLAGE;
import City.main.map.ZoneDecoration;
import static City.main.map.ZoneDecoration.TerrainSymbol.COASTAL;
import static City.main.map.ZoneDecoration.TerrainSymbol.FARMLAND;
import static City.main.map.ZoneDecoration.TerrainSymbol.FOREST;
import static City.main.map.ZoneDecoration.TerrainSymbol.ICE;
import static City.main.map.ZoneDecoration.TerrainSymbol.MARSH;
import static City.main.map.ZoneDecoration.TerrainSymbol.MOUNTAIN;
import static City.main.map.ZoneDecoration.TerrainSymbol.VOLCANO;
...
import static City.main.politics.PolitcalView.NONE;
...
public class MapRenderer {
    ...
    private java.util.Set<String> pickerValidZoneIds = null;

    private static final Color COLOR_PICKER_INELIGIBLE = new Color(70, 70, 75);
    private static final Color COLOR_PICKER_ELIGIBLE_TINT = new Color(120, 200, 100, 60);
    private City.ui.map.MercenaryArmyRenderer mercenaryArmyRenderer;

    public City.ui.map.MercenaryArmyRenderer getMercenaryArmyRenderer() {
        return mercenaryArmyRenderer;
    }
    ...
    public BarbArmyRenderer getBarbArmyRenderer() {
        return barbArmyRenderer;
    }
    ...
    /**
     * Activates zone-picker grey-out mode. Zones not in validIds are rendered
     * in flat grey regardless of current view mode; zones in validIds get a
     * faint green highlight tint so they read as selectable.
     */
    public void setPickerValidZoneIds(java.util.Set<String> validIds) {
        this.pickerValidZoneIds = validIds;
    }
    ...
    private void drawZoneFill(Graphics2D g2, Zone zone, Zone selected, Zone hovered) {
        ...
        if (pickerValidZoneIds != null && !pickerValidZoneIds.contains(zone.getId())) {
            Color base = zone == hovered ? COLOR_PICKER_INELIGIBLE.brighter() : COLOR_PICKER_INELIGIBLE;
            g2.setColor(base);
            g2.fillPolygon(poly);
            return;
        }
        ...
        switch (viewMode) {
            case POLITICAL -> {
                ...
                if (isBarbarian) {
                    ...
                    if (zone == selected) {
                        primary = primary.brighter();
                        secondary = secondary.brighter();
                    }
                } else {
        ...
        if (pickerValidZoneIds != null && pickerValidZoneIds.contains(zone.getId())) {
            g2.setColor(COLOR_PICKER_ELIGIBLE_TINT);
            g2.fillPolygon(poly);
        }
